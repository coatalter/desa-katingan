...
@section('page-title', 'Tambah Potensi')
...
@section('content')
    <div class="max-w-3xl">
        <div class="flex items-center gap-4 mb-6">
            ...
            <h1 class="text-xl font-bold text-gray-900">Tambah Potensi Desa</h1>
        </div>

        <form action="{{ route('admin.potensi.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="card">
                <div class="card-body space-y-4">
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-end">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 text-primary-600 rounded border-gray-300">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="4" class="input-field" required>{{ old('deskripsi') }}</textarea>
                    </div>

                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            <label class="input-label">Nama Pemilik/Pengelola</label>
                            <input type="text" name="pemilik" value="{{ old('pemilik') }}" class="input-field">
                        </div>
                        <div>
                            <label class="input-label">Kontak (HP/WA)</label>
                            <input type="text" name="kontak" value="{{ old('kontak') }}" class="input-field">
                        </div>
                    </div>

                    <div>
                        <label class="input-label">Alamat</label>
                        <textarea name="alamat" rows="2" class="input-field">{{ old('alamat') }}</textarea>
                    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="any" name="latitude" value="{{ old('latitude') }}"
                                class="input-field" placeholder="-1.857000">
                        </div>
                        <div>
                            <label class="input-label">Longitude</label>
                            <input type="number" step="any" name="longitude" value="{{ old('longitude') }}"
                                class="input-field" placeholder="113.365000">
                        </div>
                    </div>
                </div>
            </div>
